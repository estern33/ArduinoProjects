...
DeviceAddress thermometer1Address = { 
  0x28, 0x94, 0xA6, 0x84, 0x02, 0x00, 0x00, 0x44 };
String thermometer1Name = "Freezer";
int relay1Pin = 7;

bool relayOn = false;
bool relayOff = true;

float highSetPoint = 39.0f;
float lowSetPoint = 33.0f;
...
void computeRelayState(DeviceAddress deviceAddress, String deviceName)
{
  ...
  if (tempC != -127.00) 
  {
    if (DallasTemperature::toFahrenheit(tempC) > highSetPoint)
    {
      Serial.print("Turing ");
      Serial.print(deviceName);
      Serial.print(" power on, temp exceeded high setpoint of ");
    ...
    if (DallasTemperature::toFahrenheit(tempC) < lowSetPoint)
    {
      Serial.print("Turing ");
      Serial.print(deviceName);
      Serial.print(" power off, temp dropped below low setpoint of ");
      ...
      Serial.print("Turing ");
      Serial.print(deviceName);
      Serial.print(" power off, error reading temp, and don't want to freeze the kegs.\n\r");
      Serial.print(lowSetPoint);
      Serial.print(" deg F.\n\r");
      digitalWrite(relay1Pin, relayOff);
  }
